...
def open_popup(error):
    ...
    tk.Label(
        top,
        text="Invalid input parameter!",
        bg=label_bg,
        fg=label_fg,
        font=subtitle_font,
    ).pack(expand=True, fill="both")
    tk.Label(top, text=error, bg=label_bg, fg=label_fg, font=subtitle_font).pack(
...
def show_result_fields():
    global last_result, last_gradient
    bwd = grad.get()
    try:
    ...
    result_label = tk.Label(
        f_main, bg=label_bg, font=subtitle_font, text="Result:", fg=label_fg
    )
    ...
    for i, dim in enumerate(dimsj, start=0):
        ...
        for row in range(dim):
            for col in range(dim):
                ...
                l = tk.Label(
                    f, bg="skyblue", fg=label_fg, text=str(disp), borderwidth=1
                )
                element_counter += 1
    ...
    if bwd:
        grad_label = tk.Label(
            f_main, bg=label_bg, font=subtitle_font, text="Gradient:", fg=label_fg
        )
        ...
        for row in range(d[0]):
            for col in range(d[0]):
                ...
                l = tk.Label(
                    f2, bg="skyblue", fg=label_fg, text=str(disp), borderwidth=1
                )
                created_results.append(l)
                l.grid(row=row, column=col, sticky="EW")
    ...
    if last_result is not None:
        try:
            with open("result.npy", "wb") as f:
                np.save(f, last_result)
            show_message("Success", "Result saved to result.npy")
        except Exception as e:
            open_popup(f"Error saving result: {e}")
    else:
        open_popup("No result to save.")
...
def save_gradient():
    ...
    if last_gradient is not None:
        try:
            with open("gradient.npy", "wb") as f:
                np.save(f, last_gradient)
            show_message("Success", "Gradient saved to gradient.npy")
        except Exception as e:
            open_popup(f"Error saving gradient: {e}")
    else:
        open_popup("No gradient to save.")
...
greeting = tk.Label(
    scrollable_frame,
    font=title_font,
    text="Amazing interferometer calculator",
    background=label_bg,
    fg=label_fg,
).pack(fill="both", expand=True)
e1 = tk.Entry(cutoff_frame, font=subtitle_font, background=entry_color, fg=label_fg)
e3 = tk.Entry(
    interferometer_frame, font=subtitle_font, background=entry_color, fg=label_fg
)

tk.Label(
    cutoff_frame, font=subtitle_font, background=label_bg, fg=label_fg, text="cutoff: "
).pack(fill="both", expand=True)
e1.pack(fill="both", expand=True)
tk.Label(
    interferometer_frame,
    font=subtitle_font,
    background=label_bg,
    fg=label_fg,
    text="interferometer file name: ",
).pack(fill="both", expand=True)
...
tk.Button(input_frame, text="Show output", command=show_result_fields).pack(
    side="left", fill="both", anchor="center", padx=(10, 5), pady=5
)
tk.Button(input_frame, text="Delete output", command=delete_output).pack(
    side="left", fill="both", anchor="center", padx=5, pady=5
)
tk.Button(input_frame, text="Save result", command=save_output).pack(
    side="right", anchor="center", fill="both", padx=(5, 10), pady=5
)
tk.Button(input_frame, text="Save gradient", command=save_gradient).pack(
    side="right", anchor="center", fill="both", padx=5, pady=5
)
